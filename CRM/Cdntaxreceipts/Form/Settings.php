...
class CRM_Cdntaxreceipts_Form_Settings extends CRM_Core_Form {
  ...
  function processReceiptOptions($mode) {
    if ( $mode == 'build' ) {
      ...
      Civi::settings()->set('receipt_serial', 1]);
  ...
  function processSystemOptions($mode) {
	...
	'enable_advanced_eligibility_report' => 0,
      ...
      $defaults['delivery_method'] => Civi::settings()->get('delivery_method'),
      $defaults['attach_to_workflows'] => Civi::settings()->get('attach_to_workflows'),
      $defaults['enable_advanced_eligibility_report'] => Civi::settings()->get('enable_advanced_eligibility_report'),
      return $defaults;
    }
    else if ( $mode == 'post' ) {
      $values = $this->exportValues();
